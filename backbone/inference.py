...
class Inference:
    """
    An inference wrapper for pubfig-makeup transfer.
    It takes two image `source` and `reference` in,
    and transfers the pubfig-makeup of reference to source.
    """
    def __init__(self, config, device, model_path="GAN/G.pth"):
        """
        Args:
    ...
    def transfer(self, source: Image, reference: Image, with_face=False, mode="train"):
        """
        Args:
            source (Image): The image where pubfig-makeup will be transfered to.
            reference (Image): Image containing targeted pubfig-makeup.
        Return:
        ...
        result = self.solver.test(*source_input, *reference_input, mode=mode)
