...
class Generator(nn.Module):
    ...
    def __init__(self):
        ...
        # -------------------------- PNet(MDNet) for obtaining pubfig-makeup matrices --------------------------
        ...
        self.simple_spade = GetMatrix(curr_dim, 1)  # get the pubfig-makeup matrix

        for i in range(3):
            setattr(self, f'pnet_bottleneck_{i + 1}', ResidualBlock(dim_in=curr_dim, dim_out=curr_dim, net_mode='p'))

        # --------------------------- TNet(MANet) for applying pubfig-makeup transfer ----------------------------
    ...
    def atten_feature(mask_s, weight, gamma_s, beta_s, atten_module_g, atten_module_b):
        """
        feature size: (test, c, h, w)
        mask_c(s): (3, test, h, w)
        diff_c: (test, 138, 256, 256)
        return: (test, c, h, w)
        """
    ...
    def forward(self, c, s, mask_c, mask_s, diff_c, diff_s, gamma=None, beta=None, ret=False):
        ...
        mask_list_c: lip, skin, eye. (b, test, h, w)
        """
            ...
            if i == 3:
                if gamma is None:  # not in test_mix
                    ...
                    if ret:
                        return [gamma, beta]
                # else:                       # in test mode
                # gamma, beta = param_A[train]*w + param_B[train]*(test-w), param_A[test]*w + param_B[test]*(test-w)

                c_tnet = c_tnet * (1 + gamma) + beta  # apply pubfig-makeup transfer using pubfig-makeup matrices
            if gamma is None and i <= 2:
...
class Discriminator(nn.Module):
    ...
    def __init__(self, image_size=128, conv_dim=64, repeat_num=3, norm='SN'):
        ...
        # self.conv2 = SpectralNorm(nn.Conv2d(curr_dim, test, kernel_size=k_size, bias=False))
