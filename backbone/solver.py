...
class Solver():
    def __init__(self, config, model_path=None, target=0, target_image_dir='assets/VGGFace2/test',
                 checkpoint_path="./ckpt/GAN",
                 model_name='vgg16', dataset='vggface', adv=True, data_loader=None, inference=None):
        self.adv = adv
        ...
        if inference is not None:
            self.G.load_state_dict(torch.load(inference, map_location='cuda:0'))
            self.G = self.G.to(self.device).eval()
            return

        self.target_image_dir = os.path.join(target_image_dir, str(target))
    ...
    def cal_adv_loss(self, source, target, target_model):
        input_size = target_model[0]
        fr_model = target_model[1]
    ...
    def build_model(self):
        ...
        self.target_model = []
        self.target_model.append(get_size(self.dataset))
        fr_model = get_model(self.model_name, self.dataset)
        if self.model_path:
            fr_model.load_state_dict(torch.load(self.model_path))
            fr_model.to(self.device)
            fr_model.eval()
            self.target_model.append(fr_model)
    ...
    def test(self, real_A, mask_A, diff_A, real_B, mask_B, diff_B, mode="train"):
        cur_prama = None
        with torch.no_grad():
    ...
    def vis(self):
        ...
        for self.e in range(start, self.num_epochs):
            for self.i, (source_input, reference_input) in enumerate(self.data_loader_train):
                ...
                self.vis_train([image_s, image_r, fake_A, fake_B, purified_A, purified_B])
        ...
        for self.e in range(start, self.num_epochs):
            for self.i, (source_input, reference_input) in enumerate(self.data_loader_train):
                ...
                if (self.i + 1) % self.g_step == 0:
                    ...
                    loss_idt_A = self.criterionL1(idt_A, image_s) + self.LPIPS(idt_A, image_s) + self.LPIPS(fake_A,
                                                                                                            image_s)
                    loss_idt_B = self.criterionL1(idt_B, image_r) + self.LPIPS(idt_B, image_r) + self.LPIPS(fake_B,
                                                                                                            image_r)
                    ...
                    g_B_loss_his = 0
                    # lip
                    g_A_lip_loss_his = self.criterionHis(fake_A, image_r, mask_s[:, 0], mask_r[:, 0],
                                                         self.device) * self.lambda_his_lip
                    g_B_lip_loss_his = self.criterionHis(fake_B, image_s, mask_r[:, 0], mask_s[:, 0],
                                                         self.device) * self.lambda_his_lip
                    g_A_loss_his += g_A_lip_loss_his
                    g_B_loss_his += g_B_lip_loss_his
                    # skin
                    g_A_skin_loss_his = self.criterionHis(fake_A, image_r, mask_s[:, 1], mask_r[:, 1],
                                                          self.device) * self.lambda_his_skin
                    g_B_skin_loss_his = self.criterionHis(fake_B, image_s, mask_r[:, 1], mask_s[:, 1],
                                                          self.device) * self.lambda_his_skin
                    g_A_loss_his += g_A_skin_loss_his
                    g_B_loss_his += g_B_skin_loss_his
                    # eye
                    g_A_eye_loss_his = self.criterionHis(fake_A, image_r, mask_s[:, 2], mask_r[:, 2],
                                                         self.device) * self.lambda_his_eye
                    g_B_eye_loss_his = self.criterionHis(fake_B, image_s, mask_r[:, 2], mask_s[:, 2],
                                                         self.device) * self.lambda_his_eye
                    ...
                    if self.adv:
                        ...
                        for img_cnt in range(self.cnt_max):
                            target_img_dir = os.listdir(self.target_image_dir)
                            target_image_idx = random.randint(0, len(target_img_dir)-1)
                            target_image_name = target_img_dir[target_image_idx]

                            target_image = read_img(os.path.join(self.target_image_dir, target_image_name), 0.5,
                                                    0.5, self.device)
                            for i in range(self.diversity):
                                target_loss_A = self.cal_adv_loss(fake_A_diversity[i], target_image,
                                                                  self.target_model) * self.lambda_adv * 0.5
                                target_loss_B = self.cal_adv_loss(fake_B_diversity[i], target_image,
                                                                  self.target_model) * self.lambda_adv * 0.5
                    ...
                    if self.adv:
                        ...
                        g_loss = (loss_idt + loss_gan + loss_his + loss_reg).mean()
                ...
                loss_idt_A_h = self.criterionL1(purified_idt_A, image_s) + self.LPIPS(purified_idt_A,
                                                                                      image_s) + self.LPIPS(purified_A,
                                                                                                            image_s)
                loss_idt_B_h = self.criterionL1(purified_idt_B, image_r) + self.LPIPS(purified_idt_B,
                                                                                      image_r) + self.LPIPS(purified_B,
                                                                                                            image_r)
                ...
                h_B_loss_his = 0
                # lip
                h_A_lip_loss_his = self.criterionHis(purified_A, image_r, mask_s[:, 0], mask_r[:, 0],
                                                     self.device) * self.lambda_his_lip
                h_B_lip_loss_his = self.criterionHis(purified_B, image_s, mask_r[:, 0], mask_s[:, 0],
                                                     self.device) * self.lambda_his_lip
                h_A_loss_his += h_A_lip_loss_his
                h_B_loss_his += h_B_lip_loss_his
                # skin
                h_A_skin_loss_his = self.criterionHis(purified_A, image_r, mask_s[:, 1], mask_r[:, 1],
                                                      self.device) * self.lambda_his_skin
                h_B_skin_loss_his = self.criterionHis(purified_B, image_s, mask_r[:, 1], mask_s[:, 1],
                                                      self.device) * self.lambda_his_skin
                h_A_loss_his += h_A_skin_loss_his
                h_B_loss_his += h_B_skin_loss_his
                # eye
                h_A_eye_loss_his = self.criterionHis(purified_A, image_r, mask_s[:, 2], mask_r[:, 2],
                                                     self.device) * self.lambda_his_eye
                h_B_eye_loss_his = self.criterionHis(purified_B, image_s, mask_r[:, 2], mask_s[:, 2],
                                                     self.device) * self.lambda_his_eye
                ...
                if self.adv:
                    # loss_adv
                    self_loss_list = []
                    target_loss_A = self.cal_adv_loss(purified_A, image_s, self.target_model) * self.lambda_adv * 0.5
                    target_loss_B = self.cal_adv_loss(purified_B, image_r, self.target_model) * self.lambda_adv * 0.5
                ...
                if self.adv:
                    ...
                    h_loss = (loss_idt_h + loss_gan_h + loss_his_h + loss_reg_h).mean()
                ...
                if self.i % self.vis_step == 0:
                    ...
                    self.vis_train(
                        [mask_s[:, :, 0], image_s, image_r, idt_A, purified_idt_A, fake_A, purified_A, rec_A])
    ...
    def save_models(self):
        ...
        torch.save(self.H.state_dict(), os.path.join(self.snapshot_path, '{}_{}_H.pth'.format(self.e + 1, self.i + 1)))
