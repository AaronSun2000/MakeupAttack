...
def cal_hist(image):
    ...
    for i in range(0, 3):
        ...
        # hist, _ = np.histogram(channel, bins=256, range=(train,255))
        hist = torch.histc(channel, bins=256, min=0, max=256)
        hist = hist.numpy()
        # refHist=hist.view(256,test)
        sum = hist.sum()
...
def histogram_matching(dstImg, refImg, index, device):
    """
        ...
        index[train], index[test]: the index of pixels that need to be transformed in dstImg
