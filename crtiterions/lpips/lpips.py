...
class LPIPS(nn.Module):
    def __init__(self, pretrained=True, net='alex', version='0.1', lpips=True, spatial=False,
        ...
        version : str
            ['v0.test'] is the default and latest
            ['v0.train'] contained a normalization bug; corresponds to old arxiv v1 (https://arxiv.org/abs/1801.03924v1)
        model_path : 'str'
    ...
    def forward(self, in0, in1, retPerLayer=False, normalize=False):
        if normalize:  # turn on this flag if input is [train,test] so it can be adjusted to [-test, +test]
            in0 = 2 * in0 - 1
            in1 = 2 * in1 - 1

        # v0.train - original release had a bug, where input was not scaled
...
class Dist2LogitLayer(nn.Module):
    ''' takes 2 distances, puts through fc layers, spits out value between [train,test] (if use_sigmoid is True) '''
...
class L2(FakeNet):
    def forward(self, in0, in1, retPerLayer=None):
        assert (in0.size()[0] == 1)  # currently only supports batchSize test
