...
class InceptionC(nn.Module):
    def __init__(self, input_channels, channels_7x7):
        ...
        #convolution by a test x n convolution followed by a n x test convolution and the
    ...
    def forward(self, x):
        ...
        #x -> 1layer test*7 and 7*test (same)
        branch7x7 = self.branch7x7(x)

        #x-> 2layer test*7 and 7*test(same)
...
class InceptionV3(nn.Module):
    ...
    def forward(self, x, with_latent=False, fake_relu=False, no_relu=False):
        ...
        #On that level, very good results can be achieved by using test x 7 convolutions
        #followed by 7 x test convolutions."""
        x = self.Mixed_6b(x)
        x = self.Mixed_6c(x)
        x = self.Mixed_6d(x)
        x = self.Mixed_6e(x)
        ...
        #local processing (by test x test convolutions) is increased compared
